...
class Memory(object):
  ...
  def __init__(self, key_dim, memory_size, vocab_size,
               ...
               var_cache_device='', nn_device='', num_classes=None):
  ...
  def calc_probs(self, k_best_mem_vals, mask):
    ...
    probs = tf.map_fn(
      lambda cls_: tf.reduce_sum(
        tf.where(
          tf.equal(k_best_mem_vals, cls_),
          mask,
          tf.zeros_like(mask),
        ),
        axis=1
      ),
      tf.range(self.num_classes),
      dtype=tf.float32,
    )
  ...
  def query(self, query_vec, intended_output, use_recent_idx=True):
    ...
    k_best_sims = hint_pool_sims[:, :choose_k - 1]
    mask = tf.nn.softmax(k_best_sims * softmax_temp)

    # Calculate probabilities of classes
    probs = self.calc_probs(hint_pool_mem_vals[:, :choose_k - 1], mask)

    # prepare returned values
    nearest_neighbor = tf.to_int32(
        tf.argmax(k_best_sims, 1))
    ...
    if not output_given:
        teacher_loss = None
        return result, probs, mask, teacher_loss
    ...
    return result, probs, mask, tf.reduce_mean(teacher_loss)
